# Automating Creative AI for Accessible Image Editing
This Project is collaborate with Yum Me Print to provides text-based editing or generated using simple, natural language prompts with Google Nano Banana. This project proposes a user-friendly and secure portrait editing pipeline that removes the need for manual prompting. By analysing the input image automatically and applying a verification framework, the system ensures consistent edits while preserving the subject’s identity.  This project can dividend into 4 part:

Input Image Analysis /n
Prompt Generation /n
Image Editing by using Nano banana /n
Result Verification /n
